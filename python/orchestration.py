...
class ContaminationOrchestrator:
    ...
    def _check_server_health(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.server_threshold = data.get('contamination_score_threshold', 0.80)
    ...
    def _list_s3_files(self, s3_uri: str, suffixes: List[str] = None) -> List[Tuple[str, int, str]]:
        ...
        if suffixes is None:
            # Note: The system actually only processes JSONL (newline-delimited JSON) format,
            # but users sometimes mislabel their JSONL files as .json, so we accept both extensions
            suffixes = ['.json', '.json.gz', '.json.zst', '.json.zstd', '.json.bz2', '.json.xz',
    ...
    def _get_processed_files(self) -> Set[str]:
        ...
        #     cleaned_file_tuples = self._list_s3_files(self.config.remote_cleaned_output_dir, ['.json', '.json.gz', '.json.zst', '.json.bz2', '.json.xz',
        #                                                                                      '.jsonl', '.jsonl.gz', '.jsonl.zst', '.jsonl.bz2', '.jsonl.xz'])
        ...
        for clean_marker_path in clean_markers:
            # Add all possible variants of this file (both .json and .jsonl)
            processed.add(f"{clean_marker_path}.json")
            processed.add(f"{clean_marker_path}.jsonl")
            for ext in ['.gz', '.zst', '.zstd', '.bz2', '.xz']:
                processed.add(f"{clean_marker_path}.json{ext}")
        ...
        for key in cleaned_files:
            ...
            # Expected format: basename.json[l] or basename.json[l].gz
            basename = None
            for ext in ['.json.gz', '.json.zst', '.json.bz2', '.json.xz', '.json',
                       '.jsonl.gz', '.jsonl.zst', '.jsonl.bz2', '.jsonl.xz', '.jsonl']:
            ...
            if basename:
                # Add all possible variants of this file (both .json and .jsonl)
                processed.add(f"{basename}.json")
                processed.add(f"{basename}.jsonl")
                for ext in ['.gz', '.zst', '.zstd', '.bz2', '.xz']:
                    processed.add(f"{basename}.json{ext}")
    ...
    def _collect_upload_commands(self, job: Job) -> Tuple[List[Tuple[str, str, str]], List[str]]:
        ...
        for ext in ['.gz', '.zst', '.zstd', '.bz2', '.xz']:
            if basename_no_ext.endswith('.jsonl' + ext):
                ...
                break
            elif basename_no_ext.endswith('.json' + ext):
                basename_no_ext = basename_no_ext[:-len(ext)]
                break

        # Remove .json or .jsonl to get pure base name
        if basename_no_ext.endswith('.jsonl'):
            basename_no_ext = basename_no_ext[:-6]
        elif basename_no_ext.endswith('.json'):
            basename_no_ext = basename_no_ext[:-5]
        ...
        if job.status == 'completed':
            ...
            if report_path:
                ...
                threshold = self.server_threshold or 0.80
                ...
                clean_marker_path = relative_path
                if relative_path.endswith('.jsonl'):
                    clean_marker_path = relative_path[:-len('.jsonl')]
                elif relative_path.endswith('.json'):
                    clean_marker_path = relative_path[:-len('.json')]
            ...
            if cleaned_path:
                ...
                if not original_filename.endswith('.gz'):
                    ...
                    if original_filename.endswith(('.json', '.json.zst', '.json.zstd', '.json.bz2', '.json.xz',
                                                   '.jsonl', '.jsonl.zst', '.jsonl.zstd', '.jsonl.bz2', '.jsonl.xz')):
                        # Replace the extension with .json[l].gz
