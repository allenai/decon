...
const MIN_LENGTH: usize = 20; // Minimum character length for text entries
...
pub fn refine_reference_files(dry_run: bool) -> Result<(), Error> {
    ...
    // Phase 2: Apply filters and write refined files (if not dry run)
    if !dry_run {
        println!("\nPhase 2: Applying filters and writing refined files...");
        let filter_stats = write_refined_files(&reference_files, &duplicate_map, &output_dir)?;
...
fn display_filter_stats(stats: &FilterStats) {
    println!("\n=== FILTER STATISTICS ===");
    println!("Total lines before filters: {}", stats.total_lines_before_filters);
    println!("Lines removed by min_length filter (<{} chars): {}", MIN_LENGTH, stats.lines_removed_min_length);
    println!("Total lines after all filters: {}", stats.total_lines_after_filters);
    println!("Overall reduction: {:.1}%", 
        ((stats.total_lines_before_filters - stats.total_lines_after_filters) as f64 / stats.total_lines_before_filters as f64) * 100.0
    );
}
...
fn write_refined_files(
    ...
    // Build a set of lines to keep (first occurrence of each unique line)
    ...
    let pbar = build_pbar(reference_files.len(), "Writing refined files");
    ...
    pbar.finish_with_message("Refinement complete");
...
fn write_refined_file(
    ...
    // Process lines with deduplication and filtering
    let mut kept_count = 0;
    let mut total_count = 0;
    let mut removed_by_filter = 0;
    ...
    total_lines_before.fetch_add(total_count, Ordering::Relaxed);
    total_lines_after.fetch_add(kept_count, Ordering::Relaxed);
    
    let removed_total = total_count - kept_count;
    if removed_total > 0 {
        println!(
            "  {} -> {} ({} kept, {} removed: {} duplicates, {} min_length)",
            filename,
            output_path.display(),
            kept_count,
            removed_total,
            removed_total - removed_by_filter,
            removed_by_filter
        );
    }
