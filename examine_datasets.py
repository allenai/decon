    ...
    def try_load_dataset(dataset_path):
        ...
        try:
            ...
            for split_name in dataset.keys():
                try:
                    # Get a sample to examine fields
                    sample = next(iter(dataset[split_name]))
                    fields = list(sample.keys())
                    results["configs"]["default"][split_name] = fields
                        ...
                        try:
                            ...
                            for split_name in dataset.keys():
                                try:
                                    sample = next(iter(dataset[split_name]))
                                    fields = list(sample.keys())
                                    results["configs"][config_name][split_name] = fields
        ...
        if result["status"] == "success":
            ...
            for config_name, splits in result["configs"].items():
                if isinstance(splits, dict):
                    ...
                    for split_name, fields in splits.items():
                        if isinstance(fields, list):
                            print(f"      {split_name}: {len(fields)} fields")
                            print(f"        Fields: {', '.join(fields)}")
                        else:
                            print(f"      {split_name}: {fields}")
                else:
